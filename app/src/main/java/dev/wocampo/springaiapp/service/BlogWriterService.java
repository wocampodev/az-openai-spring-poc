...
/**
 * This service demonstrates the Evaluator-Optimizer agent pattern using Spring
 * AI.
 *
 * The pattern involves multiple AI agents working together to iteratively
 * improve content: 1. Writer agent - Creates the initial draft and refines
 * based on feedback 2. Editor agent - Evaluates the draft and provides
 * actionable feedback
 *
 * This iterative refinement continues until the content is approved or reaches
 * max iterations.
 * 
 */
@Service
public class BlogWriterService {
     ...
     *
     * The SimpleLoggerAdvisor automatically logs all AI interactions (prompts
     * and responses) when the application's logging level is set to DEBUG for
     * the advisor package.
     *
    ...
    /**
     * Generates a concise blog post (max 10 sentences) using the
     * Evaluator-Optimizer agent pattern.
     *
     * The method uses multiple AI agents to: 1. Generate an initial draft 2.
     * Evaluate the draft for quality and brevity 3. Provide feedback for
     * improvement 4. Refine the draft based on feedback 5. Repeat until
    ...
    /**
     * Enhanced version of generateBlogPost that also returns metadata about the
     * generation process.
     *
     * This method ensures at least one feedback-improvement cycle occurs to
     * demonstrate the full evaluator-optimizer pattern in action, regardless of
    ...
    /**
     * Helper method to extract actionable feedback from the Editor agent's
     * evaluation. This extracts the text after "NEEDS_IMPROVEMENT" to get just
    ...
    /**
     * Helper method to estimate token usage as we can't directly access it This
     * is a rough estimation: approximately 4 characters per token
     */
